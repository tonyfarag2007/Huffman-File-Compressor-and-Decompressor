...
- Custom binary header format designed for minimal overhead
- Bitwise packing using shift and OR operations to write sub-byte data
- Recursive tree traversal for code generation
- `std::priority_queue` with a custom comparator for the min-heap
- Manual memory management with recursive `delete` via destructor chaining
- `std::filesystem` for cross-platform path and extension handling
