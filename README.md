...
A lossless file compression and decompression tool built in C++ that implements Huffman coding to compress and decompress any file type, including binary files like images.
...
- **Compress any file type**: works on text files, images (such as BMP), and any other binary file
...
- **Minimum file size enforcement**: rejects files under 1000 bytes, since the header overhead would make the compressed file larger than the original given the small size
